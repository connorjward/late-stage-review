...
\begin{figure}
  ...
  \begin{subfigure}{0.45\textwidth}
    ...
    \begin{tikzpicture}
      ...
      \tkzSetUpStyle[
        postaction=decorate,
        decoration={markings, mark=at position .52 with {\arrow[very thick]{#1}} }
      ]{myarrow}
      ...
      \node at (e2d1) {$b$};
    \end{tikzpicture}
    \caption{Canonical orientation for unoriented \glspl{dof}.}
  ...
  \begin{subfigure}{0.45\textwidth}
    ...
    \begin{tikzpicture}
      ...
      \node at (e2d1) {$b$};
    \end{tikzpicture}
    \caption{Triangle with ...} % orientation aware?
    % from basix: "the direction of the vector used to define the DOF"
    \label{fig:orient_vector_flip}
  \end{subfigure}

  \caption{Example orientations of a triangular element with 2 \glspl{dof} per edge.}
  \label{fig:orient}
\end{figure}
...
In particular this has been shown to work for simplices~\cite{rognesEfficientAssemblyDiv2010}, as well as quadrilaterals and (some) hexes~\cite{agelekOrientingEdgesUnstructured2017,homolyaParallelEdgeOrientation2016}.
\dots
