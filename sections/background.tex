...
\subsection{Existing software for stencil computations}
...
Lastly, Simit represents unstructured meshes using \textit{hypergraphs}.
Like Ebb, distributed memory parallelism is not supported.x
...
Firedrake can then compile this form using \gls{tsfc}~\cite{homolyaTSFCStructurePreservingForm2018} before marshalling both \pyop2 and PETSc~\cite{petsc-web-page,petsc-user-ref,petsc-efficient} (a linear algebra toolkit) to automate its solution.
...
\subsection{Mesh representations}
...
\begin{figure}
  ...
  \begin{subfigure}{0.45\textwidth}
    ...
    \end{tikzpicture}
  \end{subfigure}
  \caption{An example mesh and its Hasse diagram representation.}
...
In software, a mesh is typically represented by sets of topological entities (e.g. cells, edges and vertices) along with adjacency relations between these sets.

% here

% need to emphasise the difference between DoFs, nodes and points
% and basis functions vs coefficients - ref orientation issues discussed later

% describe MOAB and others(?), then DMPlex in its own section
